...
public class CompilerServer
{
    ...
    public CompilerServer()
    {
        ...
        if (!File.Exists(_htmlPath))
        {
            ...
            _htmlPath = Path.Combine("ide_gui", "index.html");
        }
    }
    ...
    private CompileResult CompileCode(string code)
    {
        ...
        try
        {
            ...
            var syntaxErrorListener = new SyntaxErrorCollector();
            ...
            foreach (var err in syntaxErrorListener.Errors)
            {
                errors.Add(err);
            }
...
// -----------------------------------------------------------------------------
//  Custom ANTLR error listener that captures lexer and parser errors into a
//  structured list instead of printing them to stderr. Each error includes
...
/// listener interfaces.
/// </summary>
public class SyntaxErrorCollector : IAntlrErrorListener<IToken>, IAntlrErrorListener<int>
{
